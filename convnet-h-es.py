...
RANDOM_SEED = 100
# Demonstrates Assisted H-ES (r = 1)
K = 10
SHIFT_VAR = 1.0
...
if __name__ == '__main__':
    ...
    print("Created training batches in %fs." % (time.time() - start_time))

    for epoch in range(num_epochs):
        # print_perf() seems to take a long time
        print_perf(epoch, W)
        for idxs in batch_idxs:
            print("idxs: " + str(idxs))
            grad_W = loss_grad(W, train_images[idxs], train_labels[idxs])
            #print(grad_W.shape) # a 44426-element vector

            did_stoch_update = False
            if (ALG_ON):
                best_cost = frac_err(W, train_images, train_labels)
                for k in range(K):
                    # perturb the gradient
                    rand_shift = SHIFT_VAR * np.random.randn(*grad_W.shape)
                    tmp_grad_W = grad_W + rand_shift

                    # Use momentum for the stochastic update too
                    tmp_cur_dir = momentum * cur_dir + (1.0 - momentum) * tmp_grad_W
                    tmp_W = W - learning_rate * tmp_cur_dir

                    # evaluate accuracy over *test set* in the interest of time (since it's smaller)
                    tmp_cost = frac_err(W, test_images, test_labels)

                    if (tmp_cost < best_cost):
                        best_cost = tmp_cost
                        print("Stochastic update improved on base gradient.")
                        W = tmp_W
                        did_stoch_update = True


            # Perform the update if we haven't already done a stochastic one
            if not (ALG_ON and did_stoch_update):
